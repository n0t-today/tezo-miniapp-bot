...
Связанные документы: `TZ.md`, `TZ_Mini_App.md`, `TZ_Web_Admin.md`, `DB_Schema.md` (в этой же папке docs/).  
...
| **Интеграции** | Prime Hill API (loyaltyAPI, openAPI), ЮKassa, S3, Telegram Bot API |
...
| HTTP-клиент | httpx (async) | Запросы к Prime Hill, ЮKassa |
...
**Авторизация:** проверка `initData` от Telegram Web App (валидация подписи) + привязка Prime Hill. Пользователь идентифицируется по `telegram_user_id` из initData.

### 2.1 Авторизация и привязка Prime Hill
...
| POST | `/auth/unbind` | Отвязка Prime Hill | — | `{ "success": true }` |
| GET | `/auth/me` | Текущий пользователь, привязка, баланс | — | `{ "telegram_user_id": ..., "ph_bound": true, "balance": 100 }` |

**Логика `/auth/bind`:**
1. Валидация initData (Telegram).
2. Запрос к loyaltyAPI (`GuestInfo`) для поиска клиента. При отсутствии — предложение регистрации (или авторегистрация через openAPI `createClients`).
3. Сохранение привязки в БД (`prime_hill_bindings`).
4. Возврат баланса.
...
**Логика `/raffle/spin`:**
1. Проверка баланса Prime Hill (loyaltyAPI `GuestInfo`) ≥ 20 бонусов.
2. Списание 20 бонусов через openAPI (`createOrder` с `bonusWriteOff=20`, `sum=0`).
...
3. Проверка доступного баланса через loyaltyAPI (`GuestInfo`).
4. Списание бонусов через openAPI (`createOrder`).
...
**loyaltyAPI (Касса/Фронт)**
- **Назначение:** Чтение баланса клиента, проверка существования.
- **Метод:** `POST /GuestInfo/{token}` (поиск по Phone/CardNumber).
- **Авторизация:** Токен в URL пути.

**openAPI (Бэкенд)**
- **Назначение:** Списание бонусов (создание заказов), регистрация клиентов.
- **Методы:**
    - `POST /createOrder` (списание за спин/заказ).
    - `POST /createClients` (регистрация новых пользователей).
- **Авторизация:** Токен в query параметре (`?token=...`).

**Особенности:**
- При регистрации (`createClients`) использовать номер телефона как `cardBarcode`, если не выдается физическая карта.
- Списание за спин: `createOrder` с `sum=0` и `bonusWriteOff=20`.
- Ошибки API: логирование, понятные сообщения пользователю («Сервис недоступен», «Недостаточно бонусов»).
...
| INFO | `logs/info.log` | Основные события: создание заказа, спин, привязка Prime Hill, вход в админку |
...
**Перейти к следующему документу:** `TZ_Frontend.md` — ТЗ фронтенда (стек, маршруты, компоненты).
